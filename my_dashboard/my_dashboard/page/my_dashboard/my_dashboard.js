frappe.pages['my_dashboard'].on_page_load = function(wrapper) {
	...
	// create page
	//-------------
	frappe.my_dashboard.make(page);

	//Customer Section
	frappe.my_dashboard.setVisabilityOfSection(page, "Customer");
	frappe.my_dashboard.getStandardSettings(page, "Customer");

	//Sales Section
	frappe.my_dashboard.setVisabilityOfSection(page, "Sales");
	frappe.my_dashboard.getStandardSettings(page, "Sales");

	frappe.breadcrumbs.add("My Dashboard");
}
...
frappe.my_dashboard = {
	make: function(page) {
		var me = frappe.my_dashboard;
		me.page = page;
		me.body = $('<div></div>').appendTo(me.page.main);
		var data = "";
		$(frappe.render_template('my_dashboard', data)).appendTo(me.body);

		// add menu buttons
		this.page.add_menu_item(__("Change Dashboard Standard Settings"), function() {
			// navigate to global standard settings
			window.location.href="#Form/Dashboard Standard Settings";
		});
		this.page.add_menu_item(__("Add User Specific Settings"), function() {
			window.location.href="#Form/User Dashboard Settings/New User Dashboard Settings 1";
		});
		this.page.add_menu_item(__("Change User Specific Settings"), function() {
			window.location.href="#Form/User Dashboard Settings/" + frappe.user.name;
		});
		...
		this.page.main.find(".btn-bar-customer").on('click', function() {
			frappe.my_dashboard.getStandardSettings(page, "Customer", "bar");
		});
		this.page.main.find(".btn-line-customer").on('click', function() {
			frappe.my_dashboard.getStandardSettings(page,"Customer", "line");
		});
		this.page.main.find(".btn-scatter-customer").on('click', function() {
			frappe.my_dashboard.getStandardSettings(page,"Customer", "scatter");
		});
		this.page.main.find(".btn-pie-customer").on('click', function() {
			frappe.my_dashboard.getStandardSettings(page,"Customer", "pie");
		});
		this.page.main.find(".btn-percentage-customer").on('click', function() {
			frappe.my_dashboard.getStandardSettings(page,"Customer", "percentage");
		});
		...
		this.page.main.find(".btn-bar-sales").on('click', function() {
			frappe.my_dashboard.getStandardSettings(page, "Sales", "bar");
		});
		this.page.main.find(".btn-line-sales").on('click', function() {
			frappe.my_dashboard.getStandardSettings(page, "Sales", "line");
		});
		this.page.main.find(".btn-scatter-sales").on('click', function() {
			frappe.my_dashboard.getStandardSettings(page, "Sales", "scatter");
		});
		this.page.main.find(".btn-pie-sales").on('click', function() {
			frappe.my_dashboard.getStandardSettings(page, "Sales", "pie");
		});
		this.page.main.find(".btn-percentage-sales").on('click', function() {
			frappe.my_dashboard.getStandardSettings(page, "Sales", "percentage");
		});
	...
	setVisabilityOfSection: function(page, type) {
		var doctype = "Dashboard Standard Settings";
	...
	getStandardSettings: function(page, section, custom_chart_type="") {
		frappe.call({
			method: 'my_dashboard.my_dashboard.page.my_dashboard.my_dashboard.check_user_settings',
			args: {
			...
			callback: function(r) {
				if (r.message) {
					...
					frappe.call({
						...
						args: {
							doctype: "User Dashboard Settings",
							name: r.message,
						},
						callback(d) {
							if(d.message) {
								...
								frappe.my_dashboard.getMonthAsInt(page);
								
								if (custom_chart_type!="") {
									if (section == "Customer") {
										frappe.my_dashboard.defineMonthFromInt(mont_as_int.customer_from, mont_as_int.customer_to, d.message, page, section, custom_chart_type);
									}
									if (section == "Sales") {
										frappe.my_dashboard.defineMonthFromInt(mont_as_int.sales_from, mont_as_int.sales_to, d.message, page, section, custom_chart_type);
									}
								} else {
									if (section == "Customer") {
										frappe.my_dashboard.defineMonthFromInt(mont_as_int.customer_from, mont_as_int.customer_to, d.message, page, section);
									}
									if (section == "Sales") {
										frappe.my_dashboard.defineMonthFromInt(mont_as_int.sales_from, mont_as_int.sales_to, d.message, page, section);
									}
								}
							}
						}
					...
					//-------------------------------
					frappe.model.get_value('Dashboard Standard Settings', {'name': 'Dashboard Standard Settings'}, '', function(d) {
						//console.log(d);
						//return d;
						...
						if (custom_chart_type!="") {
							if (section == "Customer") {
								frappe.my_dashboard.defineMonthFromInt(mont_as_int.customer_from, mont_as_int.customer_to, d, page, section, custom_chart_type);
							}
							if (section == "Sales") {
								frappe.my_dashboard.defineMonthFromInt(mont_as_int.sales_from, mont_as_int.sales_to, d, page, section, custom_chart_type);
							}
						} else {
							if (section == "Customer") {
								frappe.my_dashboard.defineMonthFromInt(mont_as_int.customer_from, mont_as_int.customer_to, d, page, section);
							}
							if (section == "Sales") {
								frappe.my_dashboard.defineMonthFromInt(mont_as_int.sales_from, mont_as_int.sales_to, d, page, section);
							}
						}			
					 });
				}
	...
	defineMonthFromInt: function(start, end, Standard_settings, page, section, custom_chart_type="") {
		...
		if (section == "Customer"){
			if (custom_chart_type!="") {
				frappe.my_dashboard.getCustomerQTY(start, end, months, Standard_settings, page, section, custom_chart_type);
			} else {
				frappe.my_dashboard.getCustomerQTY(start, end, months, Standard_settings, page, section);
			}
		}
		if (section == "Sales"){
			if (custom_chart_type!="") {
				frappe.my_dashboard.getSalesQTY(start, end, months, Standard_settings, page, section, custom_chart_type);
			} else {
				frappe.my_dashboard.getSalesQTY(start, end, months, Standard_settings, page, section);
			}
		}
	...
	getCustomerQTY: function(start, end, months, Standard_settings, page, section, custom_chart_type="") {
		frappe.call({
			method: 'my_dashboard.my_dashboard.page.my_dashboard.my_dashboard.get_customer_qty',
			args: {
				'start': start,
				'end': end
			},
			callback: function(r) {
				if (r.message) {
					...
					if (custom_chart_type!="") {
						frappe.my_dashboard.newCustomerChart(page, Standard_settings, months, section, r.message.qty, custom_chart_type);
					} else {
						frappe.my_dashboard.newCustomerChart(page, Standard_settings, months, section, r.message.qty);
					}
				}
			}
		});
	...
	getSalesQTY: function(start, end, months, Standard_settings, page, section, custom_chart_type="") {
		frappe.call({
			method: 'my_dashboard.my_dashboard.page.my_dashboard.my_dashboard.get_sales_qty',
			args: {
				'start': start,
				'end': end
			},
